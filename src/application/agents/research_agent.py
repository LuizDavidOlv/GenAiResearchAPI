...
from src.application.models.report_models import (
    ReportState,
    ReportStateInput,
    ReportStateOutput,
)
...
class ResearchAgent:
    def __init__(self, model):
        ...
        builder.add_node(
            "generate_report_plan", generate_reports_nodes.generate_report_plan
        )
        builder.add_node("build_section_with_web_research", section_builder.compile())
        builder.add_node(
            "gather_completed_sections",
            generate_reports_nodes.gather_completed_sections,
        )
        builder.add_node(
            "write_final_sections", generate_reports_nodes.write_final_sections
        )
        builder.add_node(
            "compile_final_report", generate_reports_nodes.compile_final_report
        )
        builder.add_edge(START, "generate_report_plan")
        builder.add_conditional_edges(
            "generate_report_plan",
            section_writing_nodes.initiate_section_writing,
            ["build_section_with_web_research"],
        )
        builder.add_edge("build_section_with_web_research", "gather_completed_sections")
        builder.add_conditional_edges(
            "gather_completed_sections",
            section_writing_nodes.initiate_final_section_writing,
